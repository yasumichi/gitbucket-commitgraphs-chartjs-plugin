...
class CommitGraphsController
    extends CommitGraphsControllerBase
...
trait CommitGraphsControllerBase extends ControllerBase {
  ...
  get("/admin/commitgraphs-chartjs")(adminOnly {
    val settings = loadCommitGraphsSettings()
    html.settings(settings.CommitGraphsGitCommand, None)
  })

  post("/admin/commitgraphs-chartjs", settingsForm)(adminOnly { form =>
  ...
  get("/:owner/:repository/commitgraphs-chartjs")(referrersOnly { repository =>
    Using.resource(Git.open(getRepositoryDir(repository.owner, repository.name))) { git =>
      if (JGitUtil.isEmpty(git)) {
        html.guide(repository)
      } else {
        try {
          val commitGraphs = git.log.all.call.iterator.asScala
            .map { rev =>
              val p = Process.apply(
                Seq(
                  loadCommitGraphsSettings().CommitGraphsGitCommand,
                  "log",
                  "-n",
                  "1",
                  "--numstat",
              ...
              CommitCount(commit = new CommitInfo(rev), additions = additions, deletions = deletions)
            }
            .toSeq
            .groupBy { x =>
            ...
            .toSeq
            .sortWith((lt1, lt2) => lt1._2.lengthCompare(lt2._2.length) > 0)
            .map { case (mailAddress, ds) =>
              val dh = ds.head
              val dailys = ds
                .groupBy(x => date2DateStr(x.commit.authorTime))
                .map { case (date, ds) =>
                ...
                .toSeq
                .sortWith((lt1, lt2) => lt1.date.compareTo(lt2.date) < 0)
...
case class CommitCount(commit: CommitInfo, additions: Long, deletions: Long)
